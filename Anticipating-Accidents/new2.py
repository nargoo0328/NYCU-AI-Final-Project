...
# Network Parameters
n_input = 12544 # fc6 or fc7(1*4096)
...
n_pair_hidden = 256; # embedding pairwise object features
n_hidden = 768 # hidden layer num of LSTM
n_pair_detection = 100  
##################################################
...
def build_model():
    ...
    z = tf.placeholder("float", [None, n_frames ,n_pair_detection, n_input])  #new
    ...
    weights = {
        ...
        'em_pair': tf.Variable(tf.random_normal([n_input,n_pair_hidden], mean=0.0, stddev=0.01)),   #new
        ...
        'att_w2': tf.Variable(tf.random_normal([n_pair_hidden, 1], mean=0.0, stddev=0.01)),   #new
        'att_wa2': tf.Variable(tf.random_normal([n_hidden, n_pair_hidden], mean=0.0, stddev=0.01)),   #new
        'att_ua2': tf.Variable(tf.random_normal([n_pair_hidden, n_pair_hidden], mean=0.0, stddev=0.01)),   #new
    ...
    biases = {
        ...
        'em_pair': tf.Variable(tf.random_normal([n_pair_hidden], mean=0.0, stddev=0.01)),    #new
        'att_ba': tf.Variable(tf.zeros([n_att_hidden])),
        'att_ba2': tf.Variable(tf.zeros([n_pair_hidden])),    #new
    ...
    h_prev2 = tf.zeros([batch_size, n_hidden])      #new
    # init loss 
    loss = 0.0  
    # Mask 
    zeros_object = tf.to_float(tf.not_equal(tf.reduce_sum(tf.transpose(x,[1,2,0,3]),3),0)) # frame x n x b
    zeros_pair_object = tf.to_float(tf.not_equal(tf.reduce_sum(tf.transpose(z,[1,2,0,3]),3),0)) # frame x n x b     #new
    # Start create graph
    for i in range(n_frames):
      with tf.variable_scope('model',reuse=tf.AUTO_REUSE):
        # input features
        X = tf.transpose(x[:,i,:,:], [1, 0, 2])  # permute n_steps and batch_size (n x b x h)
        Z = tf.transpose(z[:,i,:,:], [1, 0, 2])     #new
        
        # frame embedded
        #image = tf.matmul(X[0,:,:],weights['em_img']) + biases['em_img'] # 1 x b x h

        # object embedded
        n_object = tf.reshape(X, [-1, n_input]) # (n_steps*batch_size, n_input)
        n_object = tf.matmul(n_object, weights['em_obj']) + biases['em_obj'] # (n x b) x h
        n_object = tf.reshape(n_object,[n_detection,batch_size,n_att_hidden]) # n-1 x b x h
        n_object = tf.multiply(n_object,tf.expand_dims(zeros_object[i],2))

        # object attention
        brcst_w = tf.tile(tf.expand_dims(weights['att_w'], 0), [n_detection,1,1]) # n x h x 1
        image_part = tf.matmul(n_object, tf.tile(tf.expand_dims(weights['att_ua'], 0), [n_detection,1,1])) + biases['att_ba'] # n x b x h
        ...
        ##new{
        # pairwise object embedded
        n_pair = tf.reshape(Z[:,:,:], [-1, n_input]) # (n_steps*batch_size, n_input)
        n_pair = tf.matmul(n_pair, weights['em_pair']) + biases['em_pair'] # (n x b) x h
        n_pair = tf.reshape(n_pair,[n_pair_detection,batch_size,n_pair_hidden]) # n x b x h
        n_pair = tf.multiply(n_pair,tf.expand_dims(zeros_pair_object[i],2))
        
        # object attention
        brcst_w2 = tf.tile(tf.expand_dims(weights['att_w2'], 0), [n_pair_detection,1,1]) # n x h x 1
        image_part2 = tf.matmul(n_pair, tf.tile(tf.expand_dims(weights['att_ua2'], 0), [n_pair_detection,1,1])) + biases['att_ba2'] # n x b x h
        e2 = tf.tanh(tf.matmul(h_prev2,weights['att_wa2'])+image_part2) # n x b x h
        # the probability of each object
        alphas2 = tf.multiply(tf.nn.softmax(tf.reduce_sum(tf.matmul(e2,brcst_w2),2),0),zeros_pair_object[i])
        # weighting sum
        attention_list2 = tf.multiply(tf.expand_dims(alphas2,2),n_pair)
        attention2 = tf.reduce_sum(attention_list2,0) # b x h
        ##new}

        # concat frame & object & pairwise_object
        fusion = tf.concat([attention, attention2],1)
        ...
        if i == 0:
            ...
            all_alphas2 = tf.expand_dims(alphas2,0)     #new
        else:
            ...
            temp_alphas2 = tf.expand_dims(alphas2,0)        #new
            all_alphas2 = tf.concat([all_alphas2, temp_alphas2],0)      #new
    ...
    return x,z,keep,y,optimizer,loss,lstm_variables,soft_pred,all_alphas,all_alphas2    #add z all_alphas2

def train():
    # build model
    x,z,keep,y,optimizer,loss,lstm_variables,soft_pred,all_alphas, all_alphas2 = build_model()
    ...
    for epoch in range(n_epochs):
         ...
         for batch in n_batchs:
             ...
             batch_zs = batch_data['pair_data']      #new
             ff = batch_data['labels']
             batch_ys=[]
             for fff in  ff:
               if fff==False:
                 batch_ys.append([1.,0.])
                 ...
                 batch_ys.append([0.,1.])
             batch_ys=np.array(batch_ys,dtype=np.float16)
             _,batch_loss = sess.run([optimizer,loss], feed_dict={x: batch_xs, z: batch_zs, y: batch_ys, keep: [0.5]})
             epoch_loss[batch-1] = batch_loss/batch_size
         # print one epoch
         print("Epoch:", epoch+1, " done. Loss:", np.mean(epoch_loss))
         loss_out.append(np.mean(epoch_loss))
         ...
         if (epoch+1) %5 == 0:
            ...
            print("Training")
            test_all(sess,train_num,train_path,x,z,keep,y,loss,lstm_variables,soft_pred)    #add z
            #precision_train.append(epoch_prec_train)
            print("Testing")
            test_all(sess,test_num,test_path,x,z,keep,y,loss,lstm_variables,soft_pred)      #add z
            #precision_test.append(epoch_prec_train)
    print("Optimization Finished!")
    saver.save(sess, save_path+"final_model")
    return loss_out #,precision_train,precision_test

def test_all(sess,num,path,x,z,keep,y,loss,lstm_variables,soft_pred):
    total_loss = 0.0
    ...
    for num_batch in range(1,num+1):
         ...
         test_pair = test_all_data['pair_data']     #new
         fff = test_all_data['labels']
         test_labels=[]
         for ff in  fff:
           if ff==False:
         ...
         [temp_loss,pred] = sess.run([loss,soft_pred], feed_dict={x: test_data, z: test_pair, y: test_labels, keep: [0.0]})     #add z
...
def vis(model_path):
    ...
    for num_batch in range(1,test_num):
        ...
        data = all_data['data']
        pair = all_data['pair_data']     #add  pairwise feature
        labels = all_data['labels']
        det = all_data['det']
        ID = all_data['ID']
        # run result
        [all_loss,pred,weight] = sess.run([loss,soft_pred,all_alphas], feed_dict={x: data, z:pair, y: labels, keep: [0.0]})
...
def test(model_path):
    ...
    print("model restore!!!")
    print("Training")
    test_all(sess,train_num,train_path,x,z,keep,y,loss,lstm_variables,soft_pred)
    print("Testing")
    test_all(sess,test_num,test_path,x,z,keep,y,loss,lstm_variables,soft_pred)
